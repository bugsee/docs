...
Build-timings capture ships with the `BugseeAgent` post-action in **6.1.5+**. It is **on by default** — the agent reads Xcode's `.xcactivitylog` after each Archive and uploads a categorized summary plus an optional detail blob with every build.
:::
