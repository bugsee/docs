...
Your class should implement the **BugseeDelegate** protocol and it must set itself as the delegate for Bugsee (choose the sync or async delegate method that fits your needs).
...
func bugseeAddFieldsBeforeReportCreated(_ completion: @escaping (BugseeReportFields) -> Void) {
    let reportFields = BugseeReportFields("Begin;", description: "Begin;", severity: BugseeSeverityBlocker, labels: ["qa"])
...
func bugseeCheckFields(afterReportCreated report: BugseeReportFields, completionHandler completion: @escaping (BugseeReportFields) -> Void) {
    report.summary = report.summary?.appending(";End")
    report.reportDescription = report.reportDescription?.appending(";End")
    report.severity = BugseeSeverityHigh
