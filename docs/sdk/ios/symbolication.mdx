...
| `BUGSEE_BUILD_INFO_TIMINGS_ENABLED` | **ON** | bool | Per-section build timings read from the `.xcactivitylog`. Disable so no target or section names leave the machine. |
| `BUGSEE_DEPENDENCIES_ENABLED` | **ON** | bool | Resolved CocoaPods / SPM / Carthage dependency graph. Also the gate for the OSV vulnerability scan. |
