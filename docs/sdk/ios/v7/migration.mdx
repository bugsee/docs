...
[blackout](#71-blackout) suppresses only what describes the screen — logs, network events
and traces keep being captured. If you relied on `pause` to stop *all* capture, use
`+[Bugsee stop:]` and launch again afterwards.
:::

`BugseeLifecycleEventPaused` and `BugseeLifecycleEventResumed` remain in
`BugseeLifecycleEventType` but are never dispatched in 7.x. A `bugseeLifecycleEvent:`
handler that switches on them now has unreachable branches.
...
| `BugseeLifecycleEventBeforeFeedbackShown` | _(no longer dispatched)_ |
| `BugseeLifecycleEventAfterFeedbackShown` | _(no longer dispatched)_ |
...
through. See [§8.3](#83-breadcrumbs).
...
| 6.x | Notes |
| --- | --- |
| `pause` / `resume` | Closest is `startBlackout` / `endBlackout`, which is narrower — see [§3.2](#32-pause-and-resume-are-gone) |
| `getDeviceId` | — |
| `accessToken` | — |
| `isViewHidden:` | — |
| `hideKeyboard:` | — |
...
3. **Build, and work down the errors.** Everything in §3 to §9 shows up as a build error, and the [cheat sheet](#10-cheat-sheet) covers most of them.
4. **Rewire feedback** through `BugseeFeedback` ([§4](#4-feedback-moved-to-its-own-package)) — the largest single change, and the only one that needs a new dependency.
