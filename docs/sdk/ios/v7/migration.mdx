...
:::danger[Check this before anything else]
...
here, while Android keeps it opt-in.
...
2. **Fix the option names first,** using the tables in [§2](#2-option-keys). Do this before anything else — it is the one change Xcode cannot find for you.
3. **Check the defaults that changed** ([§3](#3-defaults-that-changed)) and pin any your app relies on — nothing warns you about these, at build time or at runtime.
4. **Build, and work down the errors.** Everything in §4 to §10 shows up as a build error, and the [cheat sheet](#11-cheat-sheet) covers most of them.
5. **Rewire feedback** through `BugseeFeedback` ([§5](#5-feedback-moved-to-its-own-package)) — the largest single change, and the only one that needs a new dependency.
6. **Look at your `pause` calls by hand.** This is the one place where the obvious replacement is not quite right — blackout keeps recording logs and network traffic.
7. **(Optional)** Adopt breadcrumbs, `notify`, and the new detection options.
