...
Currently both the amount of attachments and their size are limited. Max of 3 attachments, 3MB each (size increased from 1MB starting from v3.4.0).
    ...
    func bugseeAttachments(for report: BugseeReport) -> [BugseeAttachment] {
        ...
        if (report.severity == BugseeSeverityBlocker ||
            ...
            if let data = try? Data(contentsOf: fileURL),
               let att = BugseeAttachment(name: "internal app db", filename: "custom.db", data: data) {
                // Return array with one attachment
                return [att]
            }
        }
