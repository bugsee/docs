...
For automatic log capture and the manual `Bugsee.log(...)` API, see [Logs](/sdk/android/v7/logs/). For automatic crash reporting, see [Crashes](/sdk/android/v7/crashes/).
...
For handled exceptions that should be recorded as non-terminal errors (and optionally customized with type, severity, or extra parameters), keep using `Bugsee.logException(...)` instead. Automatic crash and ANR capture does not require either API — see [Crashes](/sdk/android/v7/crashes/) for details.
