...
sidebar_position: 1
slug: "/sdk/android/v7/migration"
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

:::warning Bugsee Android SDK 7.x (beta)
This guide targets the 7.x release line, which is currently in **beta**. If you are setting up a brand-new project, start from the [7.x installation guide](/sdk/android/v7/installation) instead. For stable (6.x) installs, see the main [installation guide](/sdk/android/installation).
:::
...
The Bugsee Gradle plugin is versioned independently of the SDK runtime library. **Plugin 4.x is paired with SDK 7.x** (both currently in beta); **plugin 3.x is paired with SDK 6.x**. The two lines are not interchangeable — if you pull the 7.x SDK into a project that still references a 3.x plugin version, the build or runtime will fail. See [Gradle plugin — Requirements and compatibility](/sdk/android/v7/gradle-plugin#requirements-and-compatibility) for the supported plugin version.
:::
...
### 1.4 7.x beta DSL changes

Five DSL refactors landed mid-beta and may bite you if you started on an earlier 7.x-beta plugin. The full reference is in the [Gradle plugin guide](/sdk/android/v7/gradle-plugin#configuring-the-plugin); the changes below are the breaking ones for existing beta users.
...
See the [resulting top-level shape](/sdk/android/v7/gradle-plugin#configuring-the-plugin) in the Gradle plugin reference for the full nested structure.
...
| `Bugsee.isResumed()` | `Bugsee.isBlackout()` (inverted — returns `true` while a blackout is active). New in 7.0.0-beta7. |
...
New in 7.0.0-beta7. If your app talks to the network through a stack that the SDK does not auto-instrument (a custom HTTP client, a native bridge, hand-built request/response pairs from a desktop SDK port), you can record those exchanges into the same pipeline as auto-captured events:
...
New in 7.0.0-beta7. Inject domain-specific breadcrumbs (business checkpoints, navigation events from a wrapper SDK) into the same timeline as the SDK's built-in breadcrumb sources:
...
Custom breadcrumbs run through `setBreadcrumbFilter` like auto-captured ones — see [Privacy → Breadcrumbs](/sdk/android/v7/privacy/breadcrumbs).
...
| `AdditionalDataCapture` | `DataRequestProvider` (`com.bugsee.library.contracts.common`) |
...
| `Bugsee.isResumed()` | `Bugsee.isBlackout()` (inverted; new in 7.0.0-beta7) |
