---
title: "Build dependencies (7.x Beta)"
...
sidebar_position: 3
slug: "/sdk/android/v7/builds/dependencies"
---
...
:::caution[7.x Beta]
Build-dependencies capture is **new in 7.x**. The Gradle plugin walks the resolved classpath at task execution and uploads a normalised DFS-ordered list. The same blob feeds Bugsee's [vulnerability scan](/sdk/android/v7/builds/vulnerabilities), so capturing dependencies is also the gate for OSV-backed CVE reporting.
:::
...
The Dependencies tab is also where Bugsee's vulnerability findings show up — see the dedicated [Vulnerabilities](/sdk/android/v7/builds/vulnerabilities) page for the details. The strip renders between the chip row and the table whenever `vuln_scan_summary` is populated.
...
When `dependencies_status` flips to `'ready'`, the worker also enqueues the [vulnerability scan job](/sdk/android/v7/builds/vulnerabilities) for the same build — vulnerabilities are derived from the dependencies list, so the scan can't run before the dependencies blob lands.
