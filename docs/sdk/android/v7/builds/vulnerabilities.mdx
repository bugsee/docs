---
title: "Build vulnerabilities (7.x Beta)"
description: "Server-side OSV scan over your captured dependencies, severity buckets, manual re-scan, and advisory drill-down."
sidebar_position: 4
slug: "/sdk/android/v7/builds/vulnerabilities"
---

:::caution[7.x Beta]
Vulnerability scanning is **new in 7.x** and is fully **server-side** — the Gradle plugin does no scanning of its own and adds no DSL toggle. The scan runs against the [dependencies blob](/sdk/android/v7/builds/dependencies) the plugin uploads, so the only producer-side requirement is that dependencies capture be enabled (it is, by default).
:::
