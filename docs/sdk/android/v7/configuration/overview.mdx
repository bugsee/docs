---
title: "Configuration — Overview (7.x Beta)"
description: "Every core SDK option in Bugsee Android 7.x, with both AndroidManifest.xml meta-data and Bugsee.launch() map forms."
sidebar_position: 1
...
This page is the canonical reference for every **core SDK** option recognised by the 7.x Android SDK. Options from extension modules live on their own pages — see [NDK options](/sdk/android/v7/configuration/ndk) and [Leak options](/sdk/android/v7/configuration/leak).
...
:::note[The `LaunchOptions` class was removed in 7.x]
All configuration in 7.x flows through either the manifest or the `Map<String, Object>` passed to `launch()`. See the [6.x → 7.x migration guide](/sdk/android/v7/migration) for details.
:::
...
The full set of detection options is documented on each detector's own page in [Issue detection](/sdk/android/v7/issue-detection). The table below is the option reference.
...
For video modes and redaction details see [Privacy → Video](/sdk/android/v7/privacy/video).
...
See [Performance monitoring](/sdk/android/v7/performance) for the APM overview.
...
- **[NDK options](/sdk/android/v7/configuration/ndk)** — native crash detection (`bugsee-android-ndk`).
- **[Leak options](/sdk/android/v7/configuration/leak)** — memory and thread leak detection (`bugsee-android-leak`).
...
- [Issue detection](/sdk/android/v7/issue-detection) — per-detector deep dives.
- [Performance monitoring](/sdk/android/v7/performance) — APM transactions, spans, and sampling.
