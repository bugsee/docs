...
:::caution[7.x Beta]
Leak detection is shipped as the **`bugsee-android-leak` extension module**. The core SDK doesn't include it — see [Memory & thread leaks](/sdk/android/v7/issue-detection/leaks) for installation.
:::
...
Thread-leak detection is off by default — see [Memory & thread leaks → Thread leaks](/sdk/android/v7/issue-detection/leaks#thread-leaks) for why and when to flip it on.
