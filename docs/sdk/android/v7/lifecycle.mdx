...
Extension modules (feedback, OkHttp, Ktor, Cronet, Compose, etc.) receive their own `launch(options)` and `stop()` callbacks automatically when the SDK starts and stops — they are wired through the same `Bugsee.launch` / `Bugsee.stop` entry points and need no separate initialization from your code. See [Extensions](/sdk/android/v7/extensions) for details.
