...
:::info
The build-data pipeline is **new and substantially expanded in 7.0.0**. Build timings, dependency graphs, vulnerability scans, and size analysis all flow through the same upload — but each surface is independently gated by a DSL block and rendered on a separate dashboard tab.
:::
...
| --- | --- | --- | --- |
| **Timings** | Per-task wall-clock spans, bucketed into managed-code / native / resources / packaging / other; "vs previous build" regression deltas | Build detail → **Timings** tab | [Timings](/sdk/android/builds/timings) |
| **Dependencies** | Resolved runtime classpath as a DFS-ordered list — every library, project, and file entry with direct-vs-transitive provenance | Build detail → **Dependencies** tab | [Dependencies](/sdk/android/builds/dependencies) |
| **Vulnerabilities** | Server-side OSV.dev scan over the dependencies list, with CVSS-scored CVE findings cross-referenced against GitHub Security Advisory | Build detail → **Dependencies** tab (vuln strip above the table) | [Vulnerabilities](/sdk/android/builds/vulnerabilities) |
| **Size analysis** | AAB / APK + R8 mapping uploaded for server-side breakdown; treemap, file-level diffs, insights | Build detail → **Size** tab | [Build size analysis](/sdk/android/build-size-analysis) |
...
See the [Gradle plugin reference](/sdk/android/gradle-plugin) for the full DSL surface.
...
- **[Timings](/sdk/android/builds/timings)** — per-task wall-clock spans, categories, regression deltas, the Gantt waterfall view.
- **[Dependencies](/sdk/android/builds/dependencies)** — declared and transitive dependencies, DSL knobs for scope and selection-reason capture.
- **[Vulnerabilities](/sdk/android/builds/vulnerabilities)** — OSV scan, severity buckets, manual re-scan, advisory drill-down.
- **[Build size analysis](/sdk/android/build-size-analysis)** — the existing dedicated page for AAB / APK breakdown, baseline comparison, in-build size check.
