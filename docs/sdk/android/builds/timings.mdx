...
:::info
Build-timings capture is **new in 7.0.0**. The Gradle plugin installs a build-listener `BuildService` that records every task's wall-clock span and uploads a categorized summary plus an optional detail blob with every build.
:::
