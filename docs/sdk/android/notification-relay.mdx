...
Relay delivery is independent of an integration's Auto/Events toggles, and there is no longer a per-integration relay checkbox — selection happens in this one place. See [Webhook events → `notification.relayed`](/webhooks/events#notificationrelayed) for the payload a subscribed webhook receives.
