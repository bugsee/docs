...
description: "How to subscribe to Bugsee SDK internal state changes and lifecycle events in your Android application."
sidebar_position: 10
slug: "/sdk/android/v6/lifecycle"
---
...
Your should call **Bugsee.setLifecycleEventsListener** and provide the **LifecycleEventListener** implementation as the only parameter to it:
...
Bugsee.setLifecycleEventsListener(new LifecycleEventListener() {
    @Override
    public void onEvent(LifecycleEventTypes eventType) {
...
```kotlin
Bugsee.setLifecycleEventsListener { eventType ->
    // Handle lifecycle event here...
}
```
...
|ReportUploadFailedWithFutureRetry|Event is dispatched after bug/error/crash report upload failed and will be retried in the future|
|ReportUploadFailed|Event is dispatched after multiple bug/error/crash report upload attempts ended with failure. It indicates no more attempts will be taken and the report will not be uploaded|
