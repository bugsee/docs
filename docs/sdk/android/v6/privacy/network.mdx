...
sidebar_position: 3
slug: "/sdk/android/v6/privacy/network"
---
...
It is your method's responsibility to clean up all user identifiable data from that structure and call ```listener.onEvent(event)``` to pass it back to Bugsee. ```listener.onEvent(event)``` can be called on a thread other than the calling thread.
