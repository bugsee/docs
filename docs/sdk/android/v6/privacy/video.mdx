...
In some rare cases you might want to conceal the whole screen and stop recording events completely.
between the calls to pause and resume.

The following APIs will come in handy — no video or touch events are gathered between the calls to `pause()` and `resume()` below:
