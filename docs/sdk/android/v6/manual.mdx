...
In addition to detection of a shake gesture or launching with notification bar item, report view can be trigger programmatically:
...
Bugsee.createReport(report -> {
    ...
    // 
    // Note: Do not trigger upload while video export is being performed!
    // 
...
It is possible to report non-fatal exceptions from code. These reports will get combined similar to crashes, and you will be provided with statistics and a breakdown by unique devices, Android versions, etc.
...
If you have a single method to log all exceptions from the entire application, you may
