...
Currently both the amount of attachments and their size are limited. Max of 3 attachments, 3MB each (size increased from 1MB starting from v3.5.0).
...
Bugsee.setReportAttachmentsProvider(new ReportAttachmentsProvider() {
	...
	public ArrayList<CustomAttachment> getAttachments(Report report) {
		...
		if (report.getType() == IssueType.Crash || report.getSeverity() == IssueSeverity.Blocker) {
