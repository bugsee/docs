...
Bugsee automatically intercepts network requests made with the following clients and libraries, which use these clients as the underlying HTTP stack. For other HTTP clients additional actions are required.
...
But if you use OkHttpClient as the underlying HTTP stack for Glide, additional actions are required.
