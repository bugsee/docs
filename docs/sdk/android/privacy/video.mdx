...
description: "Secure-area APIs in the Bugsee Android SDK 7.0: activities, views, rectangles, WebViews, Compose, and the blackout API."
sidebar_position: 2
slug: "/sdk/android/privacy/video"
---
...
[6.x video privacy page](/sdk/android/v6/privacy/video).
:::

## Disabling video

Video recording can be disabled completely via the `CaptureVideo` option —
see [configuration](/sdk/android/configuration).
...
Windows carrying the system `FLAG_SECURE` flag are protected without any Bugsee
call: they are blanked in video and screenshots, excluded from the view hierarchy,
and their input is not recorded. Since 7.1.0 this is controlled by the
[`CaptureRespectFlagSecure`](/sdk/android/configuration#capture) option,
which is enabled by default. Set it to `false` if your app marks windows
`FLAG_SECURE` broadly and you would rather keep recording them.
...
Stock password fields (a `TextField` using `PasswordVisualTransformation`,
which Compose marks with the `Password` semantics property) are redacted
**automatically** once the Compose module is on the classpath — no modifier
needed. See [Compose integration](/sdk/android/extensibility/compose) for the
full list of auto-detected field types and details.
...
(like `type="password"`). For everything else Bugsee captures inside a
WebView, see [WebView capture](/sdk/android/webview).
...
blank the video and drop touches for the duration.
...
[Lifecycle events](/sdk/android/lifecycle).
