...
sidebar_position: 16
slug: "/sdk/android/startup-overhead"
---
...
So even with **every** extension enabled the SDK adds ~140 ms on a low-end device (≈ a tenth of that device's cold start), and the bulk of that is the core — the extensions together add only ~32 ms.
...
- **Only include the extensions you actually use.** Each extension is a separate Gradle dependency (see [Bugsee extensions](/sdk/android/extensibility/bugsee-extensions)). The core SDK pulls in none of them — you opt in per feature, and you only pay the launch cost of what you add.
...
- **Bugsee APM startup tracing.** With [Performance monitoring](/sdk/android/performance) enabled and the [Gradle plugin](/sdk/android/gradle-plugin)'s `startupTier` set to `MINIMAL` or higher, the cold-start transaction's span tree includes the SDK's own `ContentProvider.onCreate` (shown as `app.startup.provider`) alongside every other initializer in your app — so you can see Bugsee's slice of launch directly in the dashboard waterfall.
...
- [Performance monitoring](/sdk/android/performance) — measure your app's own startup and runtime performance.
- [Build size analysis](/sdk/android/build-size-analysis) — track the SDK's contribution to app size.
- [Extensibility](/sdk/android/extensibility) — opt into only the features you need.
