...
description: "Every core SDK option in Bugsee Android 7.1, with both AndroidManifest.xml meta-data and Bugsee.launch() map forms."
sidebar_position: 1
...
This page is the canonical reference for every **core SDK** option recognised by the 7.1 Android SDK. Options from extension modules live on their own pages — see [NDK options](/sdk/android/configuration/ndk) and [Leak options](/sdk/android/configuration/leak).
...
| `NotifyFlushDelay` | `com.bugsee.option.config.notify-flush-delay` | int (ms) | `0` | Coalescing window before the [notification relay](/sdk/android/notification-relay) queue drains; `0` starts the drain as soon as a notification is persisted. Only affects the non-urgent, batched drain — not an urgent `notify()` skip-ahead POST. |
...
| `DetectAndReportHangSampling` | `com.bugsee.option.detect.hang.sampling` | boolean | `true` | Sample the main thread's stack during a hang and attribute the report to the derived culprit frames rather than to a single snapshot taken at the moment of reporting. Only has effect when `DetectAndReportHang` is enabled. *(7.2.0)* |
...
| `DetectAndReportExitNotRespondingAsCrash` | `com.bugsee.option.detect.exit.not_responding.as_crash` | boolean | `false` | Report ANR exits as crashes rather than as exits, so they group with crashes in the dashboard. Requires `DetectAndReportExitNotResponding`. *(7.1.0)* |
| `DetectAndReportAnrSampling` | `com.bugsee.option.detect.anr.sampling` | boolean | `true` | Sample the main thread's stack during an ANR and attribute the report to the derived culprit frames rather than to a single snapshot taken once the OS confirms the ANR. Only has effect when `DetectAndReportExitNotResponding` is enabled. *(7.2.0)* |
...
| `CaptureVideoAdaptive` | `com.bugsee.option.capture.video.adaptive` | boolean | `false` | Skip capturing new frames while nothing on screen has been redrawn, lowering CPU and battery cost on idle screens. A frame is still taken at least once per second. *(7.1.0)* |
...
| `CaptureRespectFlagSecure` | `com.bugsee.option.capture.respect-flag-secure` | boolean | `true` | Honor the platform's `FLAG_SECURE`: windows that set it are blanked in video and screenshots, excluded from the view hierarchy, and their input is dropped. An app that sets `FLAG_SECURE` broadly will see empty recordings after upgrading — set to `false` to restore pre-7.1.0 behavior. *(7.1.0)* |
...
`IssueSeverity` constants carry the numeric values 1 (`VeryLow`) through 5 (`Blocker`). The value-1 constant is named `VeryLow` here and `Low` in the other Bugsee SDKs (`BugseeSeverityLow` on iOS); both mean the same severity.
