...
The programmatic constants live in the shared contracts package, alongside every other Bugsee contract type:
...
:::info
Earlier builds exposed these keys from the module-nested `com.bugsee.library.ndk.contracts.options` package. That class still exists as a deprecated source-compatibility alias — the constants resolve to identical string values, so switching the import is a no-op at runtime — but it will be removed in a future major version.
:::
