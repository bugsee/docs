...
- **Stops loading the Compose compiler plugin into Kotlin 2.2 and 2.3.** A correction to the bound
  shipped in 4.0.3, which admitted two Kotlin lines that abort the build (see 4.0.5 for the
  underlying cause). On Kotlin 2.2+ this release disables Compose tag and secure-modifier injection
  with a warning rather than failing. 4.0.5 restores the injection on those lines, so there is no
  reason to stay on 4.0.4.
