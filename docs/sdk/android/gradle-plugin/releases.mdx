...
- **Compose instrumentation is skipped on unverified Kotlin versions.** The bundled Compose compiler
  ...
  (currently anything above 2.3), Compose tag and secure-modifier injection is now disabled with a
  warning instead, and the build succeeds. Kotlin 2.1 through 2.3 are unaffected. Support for 2.4
