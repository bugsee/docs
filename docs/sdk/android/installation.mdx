...
:::note
This page documents Bugsee Android SDK 7.x. For the previous release, see the [6.x installation guide](/sdk/android/v6/installation).
:::
...
The dynamic-version range used for the auto-pulled core is bounded to the next major version above the plugin's `sdk-min-version` (currently `[7.1.0, 8.0.0)`), so you pick up any newer compatible `7.x` release automatically.
