...
| _(not available)_ | `NdkOptions.DetectAndReport` (in the `bugsee-android-ndk` module — see [NDK options](/sdk/android/configuration/ndk)) |
...
- The helper getters/setters on `NetworkEvent` are largely the same as on `BugseeNetworkEvent`, but the no-body reason getter is renamed: 6.x `getNoBodyReason()` becomes `getBodyAbsenceReason()`, returning a `NetworkEvent.NetworkBodyAbsenceReason`.
...
| `IssueSeverity` (`com.bugsee.library.data`) | `IssueSeverity` (`com.bugsee.library.contracts.options`) — same constants, same values |
