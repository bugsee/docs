...
sidebar_position: 3
slug: "/sdk/android/extensibility/custom-extensions"
---
...
:::note
The provider-level extension points (capture, detection, performance) are **new in 7.0.0**.
:::

The 7.0 extension framework is open. Anything Bugsee ships, your app — or an internal library, or a third-party — can ship too, using the same registration paths and the same lifecycle. Four kinds of custom extensions are supported.

If you're new to Bugsee's extensibility model, start with the [Overview](/sdk/android/extensibility) first — it explains which surface to pick for what kind of capability.
...
A more detailed authoring guide — exact manifest snippet, naming conventions, lifecycle hooks available to facades, how to interact with the capture coordinator and the appearance system — is not yet published. [Contact Bugsee support](mailto:support@bugsee.com) if you are undertaking a serious custom-extension effort and we will share it.
