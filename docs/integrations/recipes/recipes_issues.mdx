...
function create(context) {
    ...
    return {
    	...
    	// allowed properties: 'summary', 'summary_format', 'description', 'description_format',
    	// 'labels', 'priority', 'severity', 'reporter', 'project'
    	custom: {
...
The properties a recipe may override are fixed. Anything else you return is ignored, and any property you leave alone keeps the value Bugsee computed for it.

|Method|Overridable properties|
|---|---|
|`create()`|`summary`, `summary_format`, `description`, `description_format`, `labels`, `priority`, `severity`, `reporter`, `project`|
|`update()`|`summary`, `summary_format`, `description`, `description_format`, `state`, `priority`, `severity`, `labels`|

The `custom` object is merged one top-level key at a time rather than deeply: naming a key replaces whatever the mapping held for that key, and leaving a key out preserves it.

To drop a field from the pushed item entirely, rather than sending it empty, assign the special value `=$unset`:

```javascript
function create(context) {
    return {
        // omit the reporter altogether instead of sending an empty one
        reporter: '=$unset'
    };
}
```
